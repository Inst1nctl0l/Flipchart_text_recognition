...
import numpy as np
from tensorflow.keras.applications import VGG16
from tensorflow.keras.layers import Dense, LSTM, Input, Conv2D, MaxPooling2D, Bidirectional, Reshape, Dropout
from tensorflow.keras.models import Model
from tensorflow.keras.layers import experimental
def recognize_text(file_path, method):
    if method == "printed":
        return recognize_printed_text(file_path)
    elif method == "handwritten":
        return recognize_handwritten_text(file_path)
    else:
        return "Invalid method. Please choose 'printed' or 'handwritten'."

def recognize_printed_text(file_path):
    try:
...
def recognize_handwritten_text(file_path):
    def load_image(path):
    ...
    model.load_weights("/content/images/best-model.h5")
